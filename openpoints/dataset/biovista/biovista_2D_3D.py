...
class BioVista2D3D(Dataset):
    ...
    def getitem_by_class_id_year_ogc_fid_and_sample_id(self, class_id, year, ogc_fid, sample_id):
        for i in range(len(self.df)):
            row = self.df.iloc[i]
            if row['class_id'] == class_id and row['year'] == year and row['ogc_fid'] == ogc_fid and row[
                'sample_id'] == sample_id:
                idx = i
                break
    ...
    def append_height_feature(self, data, points):
        point_heights = points[:, self.gravity_dim:self.gravity_dim + 1] - points[
    ...
    def get_points_by_row(self, row):
        ...
        points = self.memory_pts.get(fn, None)
        if points is None:
    ...
    def get_orthophoto_by_row(self, row):
        ...
        image_array = self.memory_img.get(rgb_fn, None)
        ...
        if self.orthophoto_transform:
            ...
            # Apply circular mask
        image_array = self.apply_circlular_mask_on_orthophoto(img_array=image_array,
                                                              diameter_px=self.test_plot_diameter_pxs)
...
if __name__ == "__main__":
    ...
    dataset = BioVista2D3D(data_root='/home/simon/data/BioVista/Forest-Biodiversity-Potential/samples.csv',
